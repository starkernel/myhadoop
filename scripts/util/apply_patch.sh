...
if git apply --reverse --check "$PATCH_FILE" >/dev/null 2>&1; then
  ...
  if git apply --check "$PATCH_FILE" >"$LOG_FILE" 2>&1; then
    # 正式应用
    if git apply "$PATCH_FILE" >"$LOG_FILE" 2>&1; then
      echo "补丁：$PATCH_FILE 已经成功执行"
    else
      echo "补丁：$PATCH_FILE 执行失败（apply 阶段）"
      echo "---- git apply 输出 ----"
      cat "$LOG_FILE"
      exit 2
    fi
    ...
    if grep -qiE "does not exist|not found" "$LOG_FILE"; then
      echo "补丁：$PATCH_FILE 是新增文件，跳过"
    else
      echo "补丁：$PATCH_FILE 执行失败（check 阶段）"
      echo "---- git apply --check 输出 ----"
      cat "$LOG_FILE"
      exit 2
    fi
  fi
